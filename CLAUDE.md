...
python source/rank_sectors.py                         # STEP 1 (weekly+monthly): rank the 41 tracked sectors
...
```
data/indices/<year>/ind_close_all_*.csv ──→ rank_sectors.py ──→ journal/<…>/sectors.csv  (STEP 1: rank 41 sectors)
...
3. **`watchlist.csv` sector labels stay free-form** — there is deliberately no canonical-name check in `validate_watchlist.py`. These labels group *your stocks* for stock-selection; they are **independent of** the 41 tracked indexes that `rank_sectors.py` ranks. `rank_stocks.py` bridges the two: it resolves each label to a `SECTORS` key (handling the label ↔ exact-index-name mismatch — e.g. watchlist's `Nifty Financial Services` vs. `sectors.csv`'s `Nifty Fin Service`) and writes it to `universe.csv`'s `sector_canon` column. Labels with no matching index are left blank and warned about, never hard-failed — every current watchlist label maps to a tracked index as of the last `SECTORS` expansion, but this stays a soft warning (not a hard fail) for whenever a future label doesn't — see `docs/stock-scoring-reference.md`.
...
- `docs/` — `institutional-swing-scoring-model-v4.md` is the **current spec**; the un-suffixed file is v3, kept only for comparison. `sectors-csv-reference.md` = quick column reference for `rank_sectors.py`'s `sectors.csv` output. `Sectors.jpg` = the original 17 key NSE sectoral indexes (`rank_sectors.py`'s `SECTORS` map now tracks 41, see Model invariants above).
...
- `watchlist.csv` — **the master input, hand-maintained.** Identity only: `symbol,sector,notes`; `symbol` must match the bhavcopy `SYMBOL`. Everything else (liquidity, returns, fundamentals, tier) is derived. Currently 90 names / 13 sectors.
...
- **Missing data is renormalized, never scored 0:** `stage_score = (earned ÷ available) × nominal_weight`, plus a per-stock data-confidence flag. Scoring missing as 0 silently biases toward large caps. `rank_stocks.py` is the first script to implement this: it renormalizes at **both** criterion level (within a stage) and stage level (across the composite `score_100`), and every scored stock carries `pts_available`, `stages_covered` (e.g. `5/9`), and `score_conf` (HIGH/MED/LOW) — the confidence flag this invariant requires. It covers Stages 2 (Sector Rotation), 5 (Institutional Activity), 6 (Relative Strength), 7 (Accumulation Structure), and 8 (Breakout/Entry) today; Stages 1/3/4/9 remain unimplemented (blocked on the Nifty-EMA regime lines and the Screener merge) — `score_100` is a projection onto the 100-point scale from what *was* measured, not a comparable full v4 score. Stage 6's 52-week-high criterion (3 pts) is deliberately left unavailable until the bhavcopy archive reaches 252 sessions (89 archived as of writing) rather than fudging a shorter window as "52-week" — it self-activates once the archive is deep enough, no code change needed.
