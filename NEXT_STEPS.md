...
10. **Optional:** a real sector/index-membership data source for the full NSE universe (not just watchlist.csv's hand-entered labels) would let `discoveries.csv` candidates pass the same sector-gated shortlist as watchlist stocks, instead of landing in a separate sector-unverified list. NSE publishes official index-constituent files; none are archived yet.
